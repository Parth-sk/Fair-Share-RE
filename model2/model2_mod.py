...
class UserAgent(Agent):
    ...
    def utility_if_choose(self, platform):
        ...
        if platform == "LTG" and self.model.levy_active and self.model.ltg_passes_levy:
            effective_pb = self.model.p_b + self.model.levy
        else:
            effective_pb = self.model.p_b

        Pb = effective_pb * q

        # content price
        if platform == "LTG":
            Pc = self.model.p_c_LTG
        else:
            Pc = self.model.p_c_STG

        # preference
        if self.type == "LTG_pref":
            v = self.model.v_LTG_pref_LTG if platform == "LTG" else self.model.v_LTG_pref_STG
        else:
            v = self.model.v_STG_pref_LTG if platform == "LTG" else self.model.v_STG_pref_STG
...
class TelecomModel(Model):
    ...
    def compute_quality(self):
        ...
        return max((self.capacity - C) / self.capacity, 0)

    def compute_revenues(self, q_ltg, n_ltg, n_stg):

        levy_payment = self.levy * q_ltg if self.levy_active else 0

        telecom_rev = self.p_b * self.total_traffic + levy_payment

        # -------- FIXED LTG REVENUE --------
        ltg_rev = self.p_c_LTG * n_ltg - levy_payment
...
class App:
    ...
    def __init__(self, root):
        ...
        tk.Button(button_frame, text="Run Step", command=self.run_step).pack(side="left")
        tk.Button(button_frame, text="Activate Levy", command=self.toggle_levy).pack(side="left")
        tk.Button(button_frame, text="Toggle Pass-through", command=self.toggle_passthrough).pack(side="left")
